...
Capture <- read_csv("nifflers_capture.csv")
Recruitment <- read_csv("nifflers_recruitment.csv")
Treasure <- read_csv("nifflers_treasure.csv")
...
Capture <- Capture %>% 
  mutate(site = case_when(
    site == "gringotts" ~ "Gringotts",
    site == "Gringotts Bank" ~ "Gringotts",
    TRUE ~ site), 
    sex = as.factor(sex)
  )

Capture %>% filter(is.na(age))
Capture %>% filter(is.na(weight_g))
...
Treasure <- Treasure %>% 
  mutate(date = ymd(date),
         site = case_when(
           site == "ForbiddenForest" ~ "Forbidden Forest",
           TRUE ~ site
         ))
